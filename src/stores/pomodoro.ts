...
export const usePomodoroStore = defineStore('pomodoro', () => {
  ...
  const startTimer = () => {
    if (!isRunning.value && !isPaused.value) {
      ...
      currentSession.value = {
        ...
        startTime: new Date(),
        duration: currentMode.value === 'work' 
          ? settings.value.workDuration 
          : settings.value.shortBreakDuration,
  ...
  const resetTimer = () => {
    ...
    isRunning.value = false
    isPaused.value = false
    currentTime.value = currentMode.value === 'work' 
      ? settings.value.workDuration 
      : settings.value.shortBreakDuration
  ...
  const completeSession = async () => {
    ...
    if (currentSession.value && authStore.currentUser) {
      ...
      try {
        await addDoc(collection(db, 'sessions'), currentSession.value)
      } catch (error) {
      ...
      if (currentMode.value === 'work') {
        ...
        currentTime.value = sessionCount.value % settings.value.longBreakInterval === 0
      ...
      if ((currentMode.value === 'break' && settings.value.autoStartBreaks) ||
